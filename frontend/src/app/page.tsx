...
export default function Home() {
  const chatModalRef = useRef<{ openChatWithQuery: (query: string) => void }>(null)
  
  // Function to handle opening the chat with a specific query
  const handleOpenChatWithQuery = (query: string) => {
    // @ts-ignore - Access the exposed method through window
    if (typeof window !== 'undefined' && window.openChatWithQuery) {
      // @ts-ignore
      window.openChatWithQuery(query)
    }
  }
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-8 bg-background text-foreground">
      <div className="z-10 w-full max-w-4xl flex flex-col items-center justify-center">
        ...
        <div className="flex gap-4 mb-8">
          <Link href="https://www.linkedin.com/in/jason-causey-cs/" target="_blank" className="inline-flex">
          ...
          <Link href="https://github.com/Ru-Zah-Jin-Zebo" target="_blank" className="inline-flex">
            <Button variant="outline" size="lg" asChild>
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 mr-2">
        ...
        <div className="w-full bg-card text-card-foreground shadow-lg rounded-lg p-6 border">
          <Resume openChatWithQuery={handleOpenChatWithQuery} />
        </div>
      </div>
